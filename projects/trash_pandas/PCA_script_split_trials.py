...
for i in range(0, len(exps)):
    ...
    if stim_type == 'spont':
        ...
        for image in images:
            trash_cache_path = os.path.join('/media/charlie/Brain2017/data/dynamic-brain-workshop/trash_cache/nsPCA/split_by_run', str(image))
            tc = TrashCache(os.path.join(trash_cache_path, 'trash_cache_manifest.json'))
            pca, behavior = PCA_batch(data_set, stim_type = 'natural_scenes', images = image, splitby = 'run')

            if len(pca.keys()) > 0:
                exp = dict(id=exp_id, vars = [dict(name='var_explained', data = pca['var_explained']),
                dict(name='corr_mat', data=behavior['corr_mat']),
                dict(name='PCs', data = pca['PCs']),
                dict(name='Principle axes', data = pca['axes']),
                dict(name='Eigenvalues', data = pca['weights']),
                dict(name='Fraction of PCs', data = pca['fraction_pcs']),
                dict(name='Experiment data', data = behavior['data']),
                dict(name ='meta_data', data = meta_data),
                dict(name='var_explained_run', data = pca['var_explained_run']),
                dict(name='corr_mat_run', data=behavior['corr_mat_run']),
                dict(name='PCs_run', data = pca['PCs_run']),
                dict(name='Principle axes_run', data = pca['axes_run']),
                dict(name='Eigenvalues_run', data = pca['weights_run']),
                dict(name='Fraction of PCs_run', data = pca['fraction_pcs_run'])
                ]
                )
